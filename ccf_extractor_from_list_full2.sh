...
#     3) obj_to_container_paths.csv   -> filespace_name, full_object_name, object_id,
...
# Final dedupe (some servers repeat IDs across pages)
if [[ -s "$MAP_FILE" ]]; then
...
# Note: -x suppresses headings, trims; -t uses ';' as terminator for the input SQL file
...
# Load CID->PATH map into awk and aggregate per OID in one pass
awk -v FS=',' -v OFS=',' -v SEP="$LIST_SEP" -v MAP="$CID2PATH" '
  BEGIN{
    while ((getline < MAP) > 0) {
      # MAP lines: CID \t PATH
      split($0, a, "\t")
  ...
  FNR==1 {next}  # skip header of obj_to_container_ids.csv
  {
    ...
    if (!(key in seen)) {
      seen[key]=1
      if (!(oid in cids)) { cids[oid]=cid; paths[oid]=cid2path[cid] }
      else {
        cids[oid]=cids[oid] SEP cid
        paths[oid]=paths[oid] SEP cid2path[cid]
      }
      # Remember presentation keys
      if (!(oid in fs_of)) { fs_of[oid]=fs; full_of[oid]=full }
    }
  }
  END{
    for (o in cids) {
      print fs_of[o], full_of[o], o, cids[o], paths[o]
    }
  }' "$OBJ2CID_DENORM" \
  | sort -t',' -k1,1 -k2,2 > "${OBJ2CNP_AGG}.tmp" && \
  { echo "filespace_name,full_object_name,object_id,container_ids,container_paths" > "$OBJ2CNP_AGG"; cat "${OBJ2CNP_AGG}.tmp" >> "$OBJ2CNP_AGG"; rm -f "${OBJ2CNP_AGG}.tmp"; }
