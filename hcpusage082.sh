...
# v0.2   Improved AIX compatibility.
# v0.3   Improved token handling.
# v0.4   XML attribute parser.
# v0.5   Added curl --resolve support.
# v0.6   Added Tenant, HCP Node and HCP Type detection.
# v0.7   Added Region detection and summary reports.
# v0.8.0 Added optional CSV generation.
# v0.8.1 CSV generated automatically with timestamped filename.
# v0.8.2 Summary reports sorted by Region and Type.
#
###############################################################################
...
do
  ...
  [ -z "$namespace" ] && namespace="$namespace_from_url"
...
END {
  for (key in used_sum) {
    ...
    printf "%s;%s;%d;%.1f;%.1f;%.1f\n",
           region, type, count[key], used_tib, total_tib, pct
  }

  if (grand_total > 0) {
    printf "ZZ_TOTAL;TOTAL;%d;%.1f;%.1f;%.1f\n",
           grand_count,
...
' "$TMP_FILE" | sort -t';' -k1,1 -k2,2 | awk -F';' '
{
  region=$1
  type=$2
...
END {
  for (key in used_sum) {
    ...
    printf "%s;%s;%s;%d;%.1f;%.1f;%.1f\n",
           region, type, hcp, count[key], used_tib, total_tib, pct
  }
}
' "$TMP_FILE" | sort -t';' -k1,1 -k2,2 -k3,3 | awk -F';' '
{
